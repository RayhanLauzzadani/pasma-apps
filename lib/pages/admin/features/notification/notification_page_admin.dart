...
// Detail komplain
import 'package:pasma_apps/pages/admin/features/dispute/admin_dispute_detail_page.dart';
...
class NotificationPageAdmin extends StatelessWidget {
  ...
  Map<String, dynamic> _notifStyle({String? type, String? title}) {
    ...
    // ---- Dispute (BARU)
    if (lowerType.contains('dispute') || lowerTitle.contains('dispute') || lowerTitle.contains('komplain')) {
      return {
        "svg": null,
        "iconData": Icons.warning_amber_rounded,
        "iconColor": const Color(0xFFFF3449),
        "bgColor": const Color(0x33FF3449),
      };
    }
  ...
  Future<void> _handleTap({
    ...
    final String? disputeId = data['disputeId'] as String?;
    ...
    // Fallback: cek berdasarkan type
    if (type.contains('dispute') || title.toLowerCase().contains('dispute') || title.toLowerCase().contains('komplain')) {
      // Jika tidak ada disputeId, redirect ke dispute list
      // ignore: use_build_context_synchronously
      Navigator.push(
        context,
        MaterialPageRoute(
    ...
    // 5) Default: tidak ada action yang cocok
    ScaffoldMessenger.of(context).showSnackBar(
