...
import 'package:pasma_apps/pages/seller/features/order/review_order/review_order_page.dart';
...
class NotificationPageSeller extends StatelessWidget {
  ...
  // Dispute notif
  static const String disputeRefundedType = 'dispute_refunded';
  static const String disputeRejectedSellerType = 'dispute_rejected_seller';
  ...
  bool _isDisputeRefund(String type) => type == disputeRefundedType;
  bool _isDisputeRejected(String type) => type == disputeRejectedSellerType;

  Color _bgColorFor(String type) {
    if (type.contains('rejected') || _isDisputeRefund(type)) return const Color(0xFFFFF2F2);
    if (type.contains('approved') || _isDisputeRejected(type)) return const Color(0xFFEFF8F1);
  ...
  Color _iconBgFor(String type) {
    if (type.contains('rejected') || _isDisputeRefund(type)) return const Color(0xFFFFE1E1);
    if (type.contains('approved') || _isDisputeRejected(type)) return const Color(0xFFD8F3DD);
  ...
  Color _iconColorFor(String type) {
    if (type.contains('rejected') || _isDisputeRefund(type)) return const Color(0xFFD32F2F);
    if (type.contains('approved') || _isDisputeRejected(type)) return const Color(0xFF2E7D32);
  ...
  IconData _iconFor(String type) {
    if (type.contains('rejected') || _isDisputeRefund(type)) return Icons.close_rounded;
    if (type.contains('approved') || _isDisputeRejected(type)) return Icons.check_rounded;
  ...
  // Stream chatNotifications: khusus sisi SELLER (sorting di client)
            ...
            builder: (context, chatSnap) {
              ...
              if (userSnap.hasData) {
                for (final doc in userSnap.data!.docs) {
                  ...
                  final isDispute = _isDisputeRefund(type) || _isDisputeRejected(type);
                  final archived = data['archived'] == true;
                  if ((isLegacy || isOrder || isDelivered || isDispute) && !archived) {
                    all.add({
                      ...data,
                      '_id': doc.id,
                      '_source': 'user',
